<template>
  <aside class="trace-panel">
    ...
    <div class="metric-grid compact">
      ...
      <div><span>模型降级</span><strong>{{ metrics.fallbacks ?? 0 }}</strong></div>
    </div>
    <div class="route-card" v-if="route.selectedAgent">
      <span>{{ route.intent || 'other' }}</span><strong>{{ route.selectedAgent }}</strong>
      <small v-if="route.profile">输出档位：{{ route.profile }}</small>
      <small v-if="route.plannedAgents?.length">专业能力：{{ route.plannedAgents.join(' → ') }}</small>
      <small v-if="route.reasonCodes?.length">决策依据：{{ route.reasonCodes.join(' / ') }}</small>
    </div>
    <ol class="timeline">
    ...
    <section class="evidence-list" v-if="calls.length">
      ...
      <article v-for="call in calls" :key="`${call.sequence}-${call.started_at_epoch}`">
        ...
        <p>{{ call.provider }} / {{ call.model }} · in {{ call.input_tokens ?? '-' }} · out {{ call.output_tokens ?? '-' }} · {{ call.status }}</p>
...
function label(item) { if (item.type === 'manager') return `Manager · ${item.status}`; if (item.type === 'agent') return `Agent · ${item.agent}`; if (item.type === 'fallback') return `降级 · ${item.from_agent} → ${item.to_agent}`; if (item.type === 'first_token') return '首 Token 到达'; return item.stage || item.type }
function detail(item) { if (item.type === 'manager') return (item.workers || []).join(' + '); if (item.type === 'fallback') return item.reason || '主 Agent 执行失败'; if (item.type === 'first_token') return formatMs(item.ttft_ms); if (item.latency_ms !== undefined) return formatMs(item.latency_ms); if (item.rounds !== undefined) return `${item.rounds} 轮 · ${item.decision}`; return item.status || '' }
</script>
