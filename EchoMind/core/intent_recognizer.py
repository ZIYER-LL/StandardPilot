...
class IntentCategory(Enum):
    STANDARD_QA          = "standard_qa"          # 标准文档问答
    TDOC_SUMMARY         = "tdoc_summary"         # TDoc / 标准文稿摘要
    GAP_ANALYSIS         = "gap_analysis"         # 标准化空白分析
    PROPOSAL_DRAFT       = "proposal_draft"       # 标准提案草稿生成
    REVIEW_DEFENSE       = "review_defense"       # 提案审查与会议攻防
    KNOWLEDGE_MANAGEMENT = "knowledge_management" # 文档导入 / 知识库管理
...
_TEMPLATES: Dict[IntentCategory, List[str]] = {
    IntentCategory.STANDARD_QA: [
        "TS 23.501 里 QoS Flow 是怎么定义的？",
        "NWDAF 在 5GC 中负责什么？",
        "PCF 和 SMF 在策略控制中分别做什么？",
    ],
    IntentCategory.TDOC_SUMMARY: [
        "帮我总结这篇 TDoc",
        "这个提案主要讲了什么？",
        "请提取这篇会议文稿的背景、方案和结论",
    ],
    IntentCategory.GAP_ANALYSIS: [
        "UE 移动场景下 AI 推理服务执行位置重选是否有标准化价值？",
        "现有 5GC 是否覆盖 AI service continuity？",
        "这个问题是标准问题还是实现问题？",
    ],
    IntentCategory.PROPOSAL_DRAFT: [
        "基于这个 Gap 生成一个 TDoc 草稿",
        "帮我写一个 SA2 提案的 Background 和 Proposal",
        "根据这些证据生成标准提案初稿",
    ],
    IntentCategory.REVIEW_DEFENSE: [
        "这个提案会上可能被怎么 challenge？",
        "帮我检查这个 Proposal 是否过于宽泛",
        "这个方案是否会被质疑为实现问题？",
    ],
    IntentCategory.KNOWLEDGE_MANAGEMENT: [
        "我要上传一批 TDoc",
        "把这份标准文档加入知识库",
        "查看当前知识库里有多少文档",
    ],
    IntentCategory.GREETING: ["你好", "你能做什么", "介绍一下 StandardPilot"],
}
...
class IntentRecognizer:
    ...
    async def _llm_recognize(
        ...
        prompt = f"""你是通信标准文稿处理任务的意图分析专家。根据示例判断用户意图，返回 JSON。
...
可选意图: {", ".join(c.value for c in IntentCategory)}"""
    ...
    def _pattern_recognize(self, message: str) -> Dict[str, Any]:
        ...
        msg = message.lower()
        patterns = {
            IntentCategory.PROPOSAL_DRAFT: ["草稿", "proposal", "提案", "background", "discussion", "conclusion", "生成", "写一个"],
            IntentCategory.REVIEW_DEFENSE: ["challenge", "质疑", "攻防", "审查", "过于宽泛", "实现问题", "scope"],
            IntentCategory.GAP_ANALYSIS: ["gap", "标准化价值", "空白", "是否覆盖", "标准问题", "ai service continuity", "重选"],
            IntentCategory.TDOC_SUMMARY: ["总结", "摘要", "tdoc", "文稿", "提取", "主要讲了什么"],
            IntentCategory.KNOWLEDGE_MANAGEMENT: ["上传", "导入", "加入知识库", "知识库", "多少文档", "upload"],
            IntentCategory.STANDARD_QA: ["ts", "tr", "3gpp", "5gc", "qos", "nwdaf", "pcf", "smf", "amf", "定义", "负责什么"],
            IntentCategory.GREETING: ["你好", "您好", "介绍一下", "你能做什么", "hello", "hi"],
        }
        best_cat, best_score = IntentCategory.OTHER, 0.0
    ...
    async def _extract_entities(self, message: str) -> Dict[str, List[str]]:
        """用 LLM 从消息中提取结构化实体。"""
        message = self._clean_text(message)
        prompt = f"""从通信标准文稿处理消息中提取实体，返回 JSON（字段值为列表，没有则为空列表）:
消息: "{message}"
格式: {{"wg":[],"release":[],"spec":[],"tdoc_id":[],"agenda":[],"company":[],"topic":[]}}"""
        prompt = self._clean_text(prompt)
        try:
            resp = await self.client.messages.create(
            ...
            return {"wg": [], "release": [], "spec": [], "tdoc_id": [], "agenda": [], "company": [], "topic": []}
    ...
    def _urgency(self, message: str, intent: IntentCategory) -> UrgencyLevel:
        ...
        for level, kws in _URGENCY_KEYWORDS.items():
            if any(kw in msg for kw in kws):
                return level
        if intent in (IntentCategory.REVIEW_DEFENSE, IntentCategory.PROPOSAL_DRAFT):
