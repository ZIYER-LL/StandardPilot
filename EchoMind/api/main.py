"""
StandardPilot 通信标准文稿处理助手 — FastAPI 入口
...
async def lifespan(app: FastAPI):
    ...
    skills_dir = os.getenv("STANDARDPILOT_SKILLS_DIR") or os.getenv("ECHOMIND_SKILLS_DIR", str(pathlib.Path(_ROOT) / "skills"))
    ...
    def knowledge_fallback(params: Dict[str, Any], context: Optional[Dict[str, Any]], error: str):
        ...
        return [{
            "title": "知识库降级结果",
            "content": f"知识库暂时不可用，未能完成对“{query}”的语义检索。请稍后重试，或补充标准文稿证据后确认。",
            "score": 0.0,
            "fallback": True,
            "error": error,
        }]
    ...
    _tool_manager.register(Tool(
        ...
        description="搜索标准文稿知识库（基于 ChromaDB 向量检索）",
    ...
    logger.info("StandardPilot 已就绪")
    yield

    await _monitor.stop()
    logger.info("StandardPilot 已关闭")


# ── FastAPI ───────────────────────────────────────────────────────────────────
app = FastAPI(
    title="StandardPilot 标准文稿处理助手",
    version="2.0.0",
    lifespan=lifespan,
    docs_url="/docs",
)
...
async def chat(req: ChatRequest):
    """
    标准文稿对话接口。完整流程：
      记忆读取 → 标准任务意图识别 → Agent 路由 → Skills 注入 → 执行 → 记忆写入
    """
...
def _should_use_knowledge(message: str) -> bool:
    ...
    if msg in greetings:
        return False
    business_keywords = [
        "3gpp", "ts", "tr", "tdoc", "t-doc", "sa2", "ran", "ct", "5gc",
        "qos", "pcf", "smf", "amf", "nwdaf", "nef", "upf", "gap", "proposal",
        "challenge", "标准", "文稿", "提案", "摘要", "总结", "知识库", "上传", "导入",
        "标准化", "空白", "机制", "会议", "agenda", "release",
    ]
...
async def search(query: str, top_k: int = 5):
    """
    标准文稿知识库检索链路：查询改写 → 并行召回 → 重排 → Top-K。
    展示 MCP 工具调用的核心亮点。
    """
    if _tool_manager is None:
        raise HTTPException(503, "服务未就绪")
...
async def add_knowledge(body: BatchDocInput):
    ...
    标准文稿会自动切片（每片 500 字）并存入 ChromaDB，ChromaDB 内置 Embedding 模型自动向量化。

    示例请求体：
    ```json
    {
      "documents": [
        {"title": "AI Service in 5GC", "content": "AI service continuity under UE mobility may require analysis of QoS, Policy Control and NWDAF evidence..."},
        {"title": "SA2 TDoc 摘要", "content": "Background、Discussion、Proposal、Conclusion..."}
      ]
    }
    ```
    """
...
async def _cli():
    print(BANNER)
    print("StandardPilot CLI — 输入 quit 退出\n")
    ...
    skill_manager = SkillManager(
        root_dir=os.getenv("STANDARDPILOT_SKILLS_DIR") or os.getenv("ECHOMIND_SKILLS_DIR", str(pathlib.Path(_ROOT) / "skills")),
        max_prompt_chars=int(os.getenv("ECHOMIND_SKILLS_MAX_PROMPT_CHARS", "5000")),
    )
    skill_manager.load()
    ...
    while True:
        ...
        print(f"\nStandardPilot [{result.agent_type.value}]: {result.response}\n")
