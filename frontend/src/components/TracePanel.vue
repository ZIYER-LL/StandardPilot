<template>
  <aside class="trace-panel">
    ...
    <div class="route-card" v-if="route.selectedAgent">
      <span>{{ route.intent || 'other' }}</span>
      <strong>{{ route.selectedAgent }}</strong>
    ...
    <section class="evidence-list" v-if="evidence.length">
      ...
      <article v-for="(item, index) in evidence" :key="`${item.title}-${index}`">
        <div><strong>{{ item.title }}</strong><span>{{ item.score ?? '-' }}</span></div>
        <p>{{ item.content }}</p>
      </article>
    </section>
  </aside>
...
function label(item) {
  if (item.type === 'agent') return `Agent · ${item.agent}`
  if (item.type === 'fallback') return `降级 · ${item.from_agent} → ${item.to_agent}`
  if (item.type === 'first_token') return '首 Token 到达'
  return item.stage || item.type
}

function detail(item) {
  if (item.type === 'fallback') return item.reason || '主 Agent 执行失败'
  if (item.type === 'first_token') return formatMs(item.ttft_ms)
  if (item.latency_ms !== undefined) return formatMs(item.latency_ms)
  return item.status || ''
}
</script>
